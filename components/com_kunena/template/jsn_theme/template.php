 ...
 * @since  K4.0
 */
class KunenaTemplateJsn_Theme extends KunenaTemplate
{
	/**
	 ...
	 * @var array
	 */
	protected $default = array('jsn_theme');
	...
	public function loadLanguage()
	{
		...
		KunenaFactory::loadLanguage('kunena_tmpl_jsn_theme');

		foreach (array_reverse($this->default) as $template)
		{
			$file = "kunena_tmpl_jsn_theme";
	...
	public function initialize()
	{
		...
		$this->compileLess('assets/less/jsn_theme.less', 'kunena.css');
		$this->addStyleSheet('kunena.css');

		$filename = JPATH_SITE . '/components/com_kunena/template/jsn_theme/assets/css/custom.css';
