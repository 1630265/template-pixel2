...
if (strpos('installer + update + upgrade', $input->getCmd('view')) === false)
{
	...
	if ( ! JSNVersion::isJoomlaCompatible(JSN_PAGEBUILDER3_REQUIRED_JOOMLA_VER))
	{
		try
		{
			$result = JSNUpdateHelper::check(JSN_PAGEBUILDER3_IDENTIFIED_NAME, JSN_PAGEBUILDER3_REQUIRED_JOOMLA_VER);

			if ($result[JSN_PAGEBUILDER3_IDENTIFIED_NAME])
			{
				$app->redirect('index.php?option=' . $input->getCmd('option') . '&view=update');
			}
			else
			{
				$app->enqueueMessage(JText::_('JSN_PAGEBUILDER3_NOT_COMPATIBLE_MSG'));
			}
		}
		catch (Exception $e)
		{
	...
	if ( ! JSNVersion::checkCompatibility(JSN_PAGEBUILDER3_IDENTIFIED_NAME, JSN_PAGEBUILDER3_VERSION))
	{
		$app->redirect('index.php?option=' . $input->getCmd('option') . '&view=update');
	}
}
