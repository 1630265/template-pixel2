...
class JSNInstallerModel extends JSNBaseModel
{
	...
	public function finalize()
	{
		...
		try
		{
			...
			// Post client information to server
			...
			{
				if (file_exists(JPATH_ROOT . '/plugins/system/jsnframework/libraries/joomlashine/client/client.php'))
				{
	...
	protected function hasUpdate($identified_name, $current_version, $requiredJoomlaVersion = JSN_PAGEBUILDER3_REQUIRED_JOOMLA_VER, $version = '')
	{
		...
		if ( ! $result)
		{
			...
			if (is_object($result))
			{
				...
				if ($result AND ! empty($requiredJoomlaVersion) AND strpos($JVersion->getShortVersion(), $requiredJoomlaVersion) === false AND ! version_compare($result->version, $current_version, '>='))
				{
					$result = false;
				}

				// Does product have newer version?
				if ($result AND (empty($requiredJoomlaVersion) OR strpos($JVersion->getShortVersion(), $requiredJoomlaVersion) !== false) AND ! version_compare($result->version, $current_version, '>'))
				{
					$result = false;
				}
			}
