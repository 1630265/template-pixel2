...
e.val(c?1:0).trigger("change")}}),a(document).on("change.form.toggler.value","[data-eb-toggler-checkbox] ~ input[type=hidden]",function(){var b=a(this),c=b.val(),d=1==c?!0:!1,e=b.siblings("[data-eb-toggler-checkbox]"),f=e.is(":checked");return d&&!f?(e.prop("checked",!0),void 0):!d&&f?(e.prop("checked",!1),void 0):void 0}),b.resolve()}),EasyBlog.require().library("dialog").script("admin/grid","shared/toggler").done(function(){}),EasyBlog.module("admin/grid",function(){var b=this;EasyBlog.Controller("Grid",{defaultOptions:{"{sortColumns}":"[data-table-grid-sort]","{ordering}":"[data-table-grid-ordering]","{direction}":"[data-table-grid-direction]","{task}":"[data-table-grid-task]","{searchInput}":"[data-table-grid-search-input]","{search}":"[data-table-grid-search]","{resetSearch}":"[data-table-grid-search-reset]","{publishItems}":"[data-table-grid-publishing]","{filters}":"[data-table-grid-filter]","{checkAll}":"[data-table-grid-checkall]","{row}":"tbody > tr","{checkbox}":"[data-table-grid-id]","{boxChecked}":"[data-table-grid-box-checked]","{checkboxWrapper}":"[data-table-grid-checkbox]",overrideCheckbox:!0}},function(a,b){return{init:function(){a.implementSortable(),a.implementPublishing()
},submitForm:function(){a.element.submit()},setTask:function(b){a.task().val(b)},setOrdering:function(b){a.ordering().val(b)},setDirection:function(b){a.direction().val(b)},updateTotalChecked:function(){var b=a.checkbox(":checked").length;a.boxChecked().val(b)},selectRow:function(a){var b=a.find("input[name=cid\\[\\]]");b.prop("checked",!0)},implementSortable:function(){a.sortColumns().implement(EasyBlog.Controller.Grid.Sort,{"{parent}":a})},implementPublishing:function(){a.publishItems().implement(EasyBlog.Controller.Grid.Publishing,{"{parent}":a})
},updateCheckbox:function(b,c){var d=!c.is(":checked"),e=c.is(":radio");e&&a.row().removeClass("is-checked"),b.toggleClass("is-checked",d),c.prop("checked",d).trigger("change")},"{checkboxWrapper} click":function(c){if(!b.overrideCheckbox){var e=c.find(a.checkbox),f=c.parents(a.row);a.updateCheckbox(f,e)}},"{checkbox} change":function(){a.updateTotalChecked()},"{row} click":function(c){if(b.overrideCheckbox){var e=c.find(a.checkbox);a.updateCheckbox(c,e)}},"{checkAll} change":function(b){var d=b.is(":checked");
a.checkbox().prop("checked",d);b.is(":checked")?a.checkbox().length:0;a.updateTotalChecked()},"{filters} change":function(){a.setTask(""),a.submitForm()},"{search} click":function(){a.submitForm()},"{resetSearch} click":function(){a.searchInput().val(""),a.submitForm()}}}),EasyBlog.Controller("Grid.Sort",{defaultOptions:{items:"[data-grid-sort-item]"}},function(a){return{"{self} click":function(){var b=a.element.data("direction"),c=a.element.data("sort");a.parent.setOrdering(c),a.parent.setDirection(b),a.parent.setTask(""),a.parent.submitForm()
...
EasyBlog.Controller("Maintenance.Database",{defaultOptions:{"{start}":"[data-start]","{progress}":"[data-progress]","{progressBox}":"[data-progress-box]","{progressBar}":"[data-progress-bar]","{progressPercentage}":"[data-progress-percentage]","{errorBox}":"[data-error]","{successBox}":"[data-success]"}},function(a){return{init:function(){},"{start} click":function(b){b.hide(),a.progress().show(),a.process()},counter:0,versions:[],process:function(){a.getStats().done(function(b){a.versions=b,a.execute()})},getStats:function(){return EasyBlog.ajax("admin/controllers/maintenance/getDatabaseStats")
},execute:function(){return void 0===a.versions[a.counter]?a.completed():(a.successBox().hide(),EasyBlog.ajax("admin/controllers/maintenance/synchronizeDatabase",{version:a.versions[a.counter]}).fail(function(b){a.errorBox().show(),a.errorBox().append('<div class="alert alert-danger">'+b+"</div>")}).always(function(){a.counter++;var b=Math.floor(a.counter/a.versions.length*100)+"%";a.progressBar().css("width",b),a.progressPercentage().text(b),a.execute()}),void 0)},completed:function(){a.progressBar().css("width","100%"),a.progressPercentage().text("100%"),a.progressBox().removeClass("progress-info").addClass("progress-success"),a.successBox().delay(1e3).fadeIn(400)
}}}),b.resolve()}),EasyBlog.module("admin/maintenance/form",function(){var b=this;EasyBlog.Controller("Maintenance.Form",{defaultOptions:{"{row}":"[data-row]"}},function(a){return{init:function(){a.runscript()},counter:0,success:0,fail:0,runscript:function(){var b=a.row().eq(a.counter);if(0===b.length)return a.completed();var c=b.data("key");EasyBlog.ajax("admin/controllers/maintenance/runscript",{key:c}).done(function(){a.setStatus(b,1),a.success++}).fail(function(){a.setStatus(b,0),a.fail++}).always(function(){a.counter++,a.runscript()
})},completed:function(){a.fail<1&&(window.location="index.php?option=com_easyblog&view=maintenance&success="+a.success)},setStatus:function(a,b){var c=a.find("[data-status]"),d=a.find("[data-icon]"),e=["label-danger","label-success","label-warning"],f=["ies-warning-2","ies-checkmark","ies-wrench-3"];for(i=0;3>i;i++)c.toggleClass(e[i],b==i),d.toggleClass(f[i],b==i)}}}),b.resolve()}),EasyBlog.module("admin/grid/ordering",function(a){var b=this;EasyBlog.Controller("Grid.Ordering",{defaultOptions:{"{moveUp}":"[data-grid-order-up]","{moveDown}":"[data-grid-order-down]",row:null}},function(b){return{init:function(){b.options.row=b.element.parents("tr")
},selectRow:function(){var c=b.options.row.find("input[name=cid\\[\\]]");a(c).prop("checked",!0)},"{moveUp} click":function(){b.selectRow(),a.Joomla("submitform",["moveUp"])},"{moveDown} click":function(){b.selectRow(),a.Joomla("submitform",["moveDown"])}}}),b.resolve()}),EasyBlog.module("admin/tabs",function(){var b=this;EasyBlog.Controller("Admin.Tabs",{defaultOptions:{"{tab}":"[data-form-tabs]","{activeTabInput}":"[data-eb-form-activetab]"}},function(a){return{"{tab} click":function(b){var d=b.attr("href").replace("#","");
a.activeTabInput().val(d)}}}),b.resolve()}),EasyBlog.module("admin/toolbar",function(a){var b=this;EasyBlog.Controller("Admin.Toolbar",{defaultOptions:{"{action}":"[data-action]",hints:{empty:""},bindings:{}}},function(b,c){return{init:function(){b.element.removeClass("hide").appendTo("#toolbar")},getTableController:function(){var b=a("[data-grid-eb]").controller();return b},getSelectedItems:function(){var b=new Array;return a("[data-table-grid]").find("input[name=cid\\[\\]]:checked").each(function(){var c=a(this).val();
b.push(c)}),b},submitForm:function(a){table=b.getTableController(),table.setTask(a),table.submitForm()},renderDialog:function(a){EasyBlog.dialog({content:EasyBlog.ajax(a),bindings:{"{submitButton} click":function(){"function"==typeof c.bindings[a].submit&&c.bindings[a].submit()}}})},"{action} click":function(a){var e=b.getSelectedItems();if(e.length<=0)return alert(c.hints.empty),void 0;var f=a.data("type"),g=a.data("action");return"post"==f?(b.submitForm(g),void 0):("dialog"==f&&b.renderDialog(g),void 0)}}}),b.resolve()
})});