...
class PlgSystemJsnExtFw extends JPlugin
{
	...
	public function __construct($subject, $option = array())
	{
		...
		$dbo->setQuery("SELECT * FROM #__extensions WHERE type = 'component' AND manifest_cache LIKE '%\"author\":\"JoomlaShine.com\"%';");

		foreach ($dbo->loadObjectList() as $product)
		{
			// Parse product params.
			$params = json_decode($product->params);

			if (isset($params->live_update_notification) && (int) $params->live_update_notification)
			{
				// Prepare checking interval.
				if (!isset($params->live_update_check_interval))
				{
					$params->live_update_check_interval = 6;
				}

				// Generate path to lock file.
				$lock = $this->app->getCfg('tmp_path') . "/jsn_update_check/{$product->element}.lock";

				if (!JFile::exists($lock) || ( time() - filemtime($lock) ) > ( (int) $params->live_update_check_interval * 60 * 60 ))
				{
					// Create/update lock file.
					if (!JFile::exists($lock))
					{
						JFile::write($lock, $product->element);
					}
					else
					{
						touch($lock);
					...
					// Check for product update.
					if ($results = JsnExtFwUpdate::check($product->element))
					{
						foreach ($results as $result)
						{
							if ($result['identified_name'] != JsnExtFwHelper::getConstant('IDENTIFIED_NAME', $product->element))
							{
								continue;
							}

							// Send email to announce super admins about product update.
							if (!$admins)
							{
								// Get Joomla config.
								$config = JFactory::getConfig();

								// Build query to get all super admin accounts.
								$dbo->setQuery(
									$dbo->getQuery(true)
										->select('u.id, u.email')
										->from('#__user_usergroup_map AS g')
										->join('INNER', '#__users AS u ON g.user_id = u.id')
										->where('g.group_id = 8')
										->where('u.sendEmail = 1')
										->where('u.block = 0'));
							...
							$substitutions = array(
								'__SITE_NAME__' => $config->get('sitename'),
								'__SITE_URL__' => JUri::root(),
								'__PRODUCT__' => $result['name'],
								'__CURRENT__' => JsnExtFwHelper::getConstant('VERSION', $product->element),
								'__NEW__' => $result['version'],
								'__LINK__' => JUri::root() . "administrator/index.php?option={$product->element}&view=about"
							);
